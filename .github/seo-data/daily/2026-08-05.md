...
- GitHub does not execute a workflow for the same event that first introduces that workflow file, so a subsequent docs-path push is required to activate the now-existing workflow.
...
- A newly introduced workflow needs a later matching event before its execution can be used as evidence.
