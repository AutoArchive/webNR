...
- Final metadata-corrected closeout Quality run `31038924037` passed and pull
  request `#45` squash-merged as
  `6f8391f6a3f06a2da70e3c203ad4b89bb241ac48`.

## Google-native exports and additional GA4 destination — 13:24 PDT

- The exact WebNR Drive folder now contains seven weekly CSV artifacts for
  `2026-07-27` through `2026-08-02`: one GA4 organic landing-page report and six
  Search Console reports.
- A production-authorized repeat export skipped all seven filenames with no
  errors; private routing IDs and raw rows remain outside Git.
- Production currently exposes only `G-DGH8HNQKE4`; the user requested the new
  WebNovel measurement `G-NL0WV2XMJN` across the site as well.
- Preserve the existing destination and owner-required full-URL reporting.
  Add the new destination to the reader and documentation outputs with the same
  `window.location.href`, pathname-plus-query, disabled Google signals, and
  disabled ad-personalization behavior.
- Update the shared skill gitlink from
  `5c3174fa6ca5b01ff2f56b1cfb248877187ce2f5` to
  `1140a11b9a366ddb611d19d691d81122184f7f9e`, which includes Google-managed
  export guidance and replaces Markdown schema enforcement with a lightweight
  safety check.
- Acceptance: both public production outputs must expose both measurement IDs,
  retain full-URL behavior, identify the exact squash commit in their build
  artifacts, and pass all expected CI before closeout.
