...
- Last completed product and reader-content change: pull request #62, squash merge `7140fd24277ab357ed8029db8aa8f6c1ecdfe6bd`
...
- `app-pages/build.json` and `gh-pages/build.json` both identify exact squash commit `7140fd24277ab357ed8029db8aa8f6c1ecdfe6bd`. The documentation mirror labels itself `github-pages-build-mirror`.
...
- Pull request #62 final Quality run `31303061815` passed Web quality, Documentation quality, Production evidence and Chromium user journeys.
...
- Production-evidence logic now follows the current topology: `app.webnovel.win` is the attributable reader target; GitHub Pages is the attributable documentation build mirror and must emit `www.webnovel.win` canonical output.
...
- `https://webnr.pages.dev/` is the reader application mirror and must not be used as the documentation origin for `www`.
...
- Repository-side canonical generation is complete. The remaining `www` serving-path change is the external Cloudflare blocker recorded in `block.md`.
...
- The currently authenticated connections do not expose write control for the `webnovel.win` Cloudflare zone. This does not block routine repository/product work, but it blocks the specific external production-routing repair for the canonical `www` hostname.
...
3. Preserve `https://app.webnovel.win/` and its source/download routes as the reader application.
